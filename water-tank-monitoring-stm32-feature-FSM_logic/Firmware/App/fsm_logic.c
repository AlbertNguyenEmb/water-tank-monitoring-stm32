...
void Fsm_InitWithConfig(const FsmConfig_t *config)
{
    ...
    /* Keep configuration inside meaningful physical ranges.  This prevents
     * an invalid threshold from creating an impossible state machine. */
    if ((fsm.config.low_level_percent < 0.0f) ||
        (fsm.config.low_level_percent > 100.0f))
    {
        fsm.config.low_level_percent = FSM_DEFAULT_LOW_LEVEL_PERCENT;
    }

    if ((fsm.config.fill_stop_percent < 0.0f) ||
        (fsm.config.fill_stop_percent > 100.0f))
    {
        fsm.config.fill_stop_percent = FSM_DEFAULT_FILL_STOP_PERCENT;
    }

    if ((fsm.config.overflow_percent < 0.0f) ||
        (fsm.config.overflow_percent > 100.0f))
    {
        fsm.config.overflow_percent = FSM_DEFAULT_OVERFLOW_PERCENT;
    }

    if ((fsm.config.overflow_clear_percent < 0.0f) ||
        (fsm.config.overflow_clear_percent > 100.0f))
    {
        fsm.config.overflow_clear_percent = FSM_DEFAULT_OVERFLOW_CLEAR_PERCENT;
    }

    if (fsm.config.fill_timeout_ms == 0U)
    {
        fsm.config.fill_timeout_ms = FSM_DEFAULT_FILL_TIMEOUT_MS;
    }

    /* Thresholds must have a physically meaningful order. */
    if (fsm.config.low_level_percent >= fsm.config.fill_stop_percent)
    {
        fsm.config.low_level_percent = FSM_DEFAULT_LOW_LEVEL_PERCENT;
        fsm.config.fill_stop_percent = FSM_DEFAULT_FILL_STOP_PERCENT;
    }

    if (fsm.config.fill_stop_percent >= fsm.config.overflow_percent)
    {
        fsm.config.fill_stop_percent = FSM_DEFAULT_FILL_STOP_PERCENT;
        fsm.config.overflow_percent = FSM_DEFAULT_OVERFLOW_PERCENT;
    }

    if (fsm.config.overflow_clear_percent >= fsm.config.overflow_percent)
    {
        fsm.config.overflow_clear_percent = FSM_DEFAULT_OVERFLOW_CLEAR_PERCENT;
    }
...
FsmOutput_t Fsm_Update(const FsmInput_t *input)
{
    ...
    case FSM_STATE_ERROR:
        ...
        {
            /* A sensor fault can only clear after a valid measurement is
             * available again.  A button press alone must never mask a bad
             * sensor. */
            if (input->sensor_ok)
            {
                Fsm_EnterState(FSM_STATE_MONITORING, input->now_ms);
